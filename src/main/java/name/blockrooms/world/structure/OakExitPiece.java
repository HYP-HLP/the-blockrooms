 ...
 * 基于 NBT 模板的橡木出口结构部件。
 *
 * <p>模板：{@code data/blockrooms/structures/oak_exit.nbt}（用结构方块导出）。
 * 模板缺失时 {@link StructureTemplateManager#getOrCreate} 返回空模板，部件什么都不生成、不会崩溃。
 * 限制：结构尺寸请保持在 16×8×16 以内（单个区块），超出部分会被包围盒裁剪。</p>
 *
 * <p>锚点（部件原点）放在 y=0，与 BlockLevel 0 世界地面（y=0 橡木木板）齐平；
 * 朝向在创建时由结构随机确定并随 NBT 序列化（跨区块生成/重载保持一致）。</p>
 */
public class OakExitPiece extends StructurePiece {
    public static final Identifier TEMPLATE_ID = Identifier.fromNamespaceAndPath(Blockrooms.MODID, "oak_exit");
    ...
    public OakExitPiece(BlockPos anchor, Rotation rotation) {
        super(ModStructures.OAK_EXIT_PIECE_TYPE.get(), 0,
                BoundingBox.fromCorners(anchor, anchor.offset(SIZE_X - 1, SIZE_Y - 1, SIZE_Z - 1)));
        this.anchor = anchor;
        this.rotation = rotation;
    }

    public OakExitPiece(StructureTemplateManager templates, CompoundTag tag) {
        super(ModStructures.OAK_EXIT_PIECE_TYPE.get(), tag);
